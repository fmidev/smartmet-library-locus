...
void search_by_iso_639_1_name()
{
  ...
  std::optional<ISO639::Entry> entry;
  entry = iso639.get("fi");
  if (!entry)
  {
...
void load_from_geonames_and_search()
{
  ...
  else
  {
    ...
    if (!entry->iso639_1 || (*entry->iso639_1 != "lv") ||
        (!entry->iso639_2 || (*entry->iso639_2 != "lav")) || (entry->iso639_3 != "lav"))
    {
  ...
  else
  {
    ...
    std::optional<ISO639::Entry> entry3 = lq.get_iso639_table()->get("deu");
  ...
  {
    if (entry->iso639_1 || entry->iso639_2 || (entry->iso639_3 != "fmisid"))
    {
      std::ostringstream tmp;
      tmp << *entry;
      TEST_FAILED("Returned language codes " + tmp.str() + " differs from expected ones");
